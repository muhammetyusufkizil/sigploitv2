...
try:
    ...
    try:
        from gtp.gtp_v2_core.path_mgmt_messages.echo import EchoResponse
        from gtp.gtp_v2_core.commons.gtp_v2_commons import GTPmessageTypeDigit
    except ImportError:
        from path_mgmt_messages.echo import EchoResponse
        from commons.gtp_v2_commons import GTPmessageTypeDigit
    ...
    def run(self):          
        ...
        while self.sock is not None and self.is_running:
            try:
                ...
                # EBADFD is Linux-specific, use EBADF on Windows
                bad_fd_error = getattr(errno, 'EBADFD', errno.EBADF)
                if e.errno == bad_fd_error:
