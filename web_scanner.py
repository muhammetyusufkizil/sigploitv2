...
def stage_dns_whois(domain, results):
    ...
    # NS Records
    ns_out = _run_cmd(f'nslookup -type=NS {domain} 2>/dev/null || dig NS {domain} +short 2>/dev/null')
    ...
    # MX Records
    mx_out = _run_cmd(f'nslookup -type=MX {domain} 2>/dev/null || dig MX {domain} +short 2>/dev/null')
    ...
    txt_out = _run_cmd(f'nslookup -type=TXT {domain} 2>/dev/null || dig TXT {domain} +short 2>/dev/null')
    ...
    # WHOIS
    whois_out = _run_cmd(f'whois {domain} 2>/dev/null', timeout=10)
    if whois_out:
...
def stage_port_scan(domain, active_subs, all_subdomains, results):
    ...
    if not _check_tool('nmap'):
        ...
        for sub_info in targets:
            ...
            open_ports = []
            ports_to_scan = list(FULL_PORT_RANGE)
...
# ============================================
# ASAMA 7: RAPOR OLUSTURMA
# ============================================

def stage_report(domain, results):
    """TXT ve JSON rapor olustur."""
    print(f"\n\033[36m{'='*60}\033[0m")
    print(f"\033[36m  ASAMA 7: Rapor Olusturma\033[0m")
    print(f"\033[36m{'='*60}\033[0m\n")
...
def web_scan_menu():
    ...
    print()
    print("  Tarama modu:")
    print("    1) Tam tarama (tum asamalar)")
    ...
    valid_modes = {'0', '1', '2', '3', '4', '5'}
    while True:
        mode = input("  Secim [1]: ").strip() or "1"
        if mode in valid_modes:
            break
        print("  [-] Geçersiz seçim. 0-5 arasında bir değer girin.")
